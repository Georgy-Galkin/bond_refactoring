...
def split_large_csvs(nameslist, extracted_path, headers, chunksize=1_000_000):
    """
    ...
    for name in nameslist:
        ...
        for file in os.listdir(base_folder):
            if "_fact_dat" in file:
                ...
                for i, chunk in enumerate(pd.read_csv(file_path, chunksize=chunksize, sep="|")):
                    output_file = os.path.join(split_folder, f"{name}_fact_data{i}.csv")
                    chunk.to_csv(output_file, sep="|", columns=headers, index=False)
